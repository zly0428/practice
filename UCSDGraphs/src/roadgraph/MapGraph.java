...
public class MapGraph {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		Comparator<MapNode> OrderIsdn =  new Comparator<MapNode>() {
		...
		PriorityQueue<MapNode> toExplore = new PriorityQueue<MapNode>(11,OrderIsdn);
		...
		while (!toExplore.isEmpty()) {
			...
			if ( !visited.contains(next)) {
				...
				for (MapNode neighbor : neighbors) {
					if (!visited.contains(neighbor)) {
						double d = next.getDistance() + next.getEdge(neighbor).getLength();
						if ( d < neighbor.getDistance() ) {
		...
		/*
		Iterator<MapNode> iterator= visited.iterator();
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		for (GeographicPoint pt : pointNodeMap.keySet())
		{
			MapNode n = pointNodeMap.get(pt);
			n.setDistance(Double.MAX_VALUE);
			n.setActualDistance(Double.MAX_VALUE);
		}
		startNode.setDistance(0.0);
		...
		while (!toExplore.isEmpty()) {
			...
			if ( !visited.contains(next)) {
				...
				for (MapNode neighbor : neighbors) {
					if (!visited.contains(neighbor)) {
						double d = next.getDistance() + next.getEdge(neighbor).getLength();
						double preDist = d + neighbor.getLocation().distance(goal);
...
/*
		...
		List<GeographicPoint> routeDij = theMap.dijkstra(new GeographicPoint(1.0,1.0), 
				 new GeographicPoint(8.0,-1.0));
		System.out.println(routeDij);	
		
		List<GeographicPoint> routeA = theMap.aStarSearch(new GeographicPoint(1.0,1.0), 
				 new GeographicPoint(8.0,-1.0));

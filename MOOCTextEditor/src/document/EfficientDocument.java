	...
	private void processText()
	{
		...
		if ( tokens.size() > 0) {
			for ( String s : tokens) {
				if ( isWord(s)) {
					countWords++;
					countSyllables += countSyllables(s);
				}
				if ( !isWord(s)) {
					countSentences++;
...
*/
	...
	protected int countSyllables(String s) {
		char[] carray = s.toLowerCase().toCharArray();
		String vowels = "aeiouy";
		int count = 0;
		boolean multipleVowels = false;
		//System.out.print("Counting syllables in " + s + "...");
		for ( int i = 0; i < carray.length; i++) {
			for ( int j = 0; j < vowels.length(); j++) {
				if ( carray[i] == (vowels.charAt(j))) {
					if ( multipleVowels == false ) {
						count++;
						multipleVowels = true;
						break;
					}
					else if ( multipleVowels == true && i == carray.length -1 && carray[i]=='e') {
						count++;
					}
					else {
						break;
				...
				if ( j == vowels.length()-1 && carray[i] != vowels.charAt(vowels.length()-1) ) {
					multipleVowels = false;
				}
			}
			if ( (i == carray.length - 1) && (count > 1) && (carray[carray.length - 1] == 'e') ) {
				count--;
			}

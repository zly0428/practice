...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for ( int i = 0; i < s.length()+1; i++) {
			for ( int charCode = (int) 'a'; charCode <= (int) 'z'; charCode++ ) {
				StringBuffer ins = new StringBuffer(s);
				if ( i < s.length()) {
					ins.insert(i, (char) charCode);
				}
				else {
					ins.append((char) charCode);
				}
				if ( !currentList.contains(ins.toString()) && (!wordsOnly||dict.isWord(ins.toString()))) {
					currentList.add(ins.toString());
				}
			}
		}
	}
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for ( int i = 0; i < s.length(); i++ ) {
			for ( int charCode = (int) 'a'; charCode <= (int) 'z'; charCode++ ) {
				StringBuffer del = new StringBuffer(s);
				del.deleteCharAt(i);
				if ( !currentList.contains(del.toString()) && (!wordsOnly||dict.isWord(del.toString()))) {
					currentList.add(del.toString());
				}
			}
		}
	}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while ( !queue.isEmpty() && numSuggestions > 0) {
			...
			subsitution(curr, curList, true);
			insertions(curr, curList, true);
			deletions(curr, curList, true );

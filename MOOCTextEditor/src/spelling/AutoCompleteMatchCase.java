...
public class AutoCompleteMatchCase implements  Dictionary, AutoComplete {
	...
	public boolean addWord(String word)
	{
		...
		for ( int i = 0; i < w.length(); i++ ) {
			if ( cur.getChild(w.charAt(i)) == null) {
				cur = cur.insert(w.charAt(i));
			}
			else {
				cur = cur.getChild(w.charAt(i));
			}
		}
		if ( cur.endsWord() == true) {
			return false;
		}
		else {
     ...
     */@Override
     public List<String> predictCompletions(String prefix, int numCompletions) 
     {
    	 ...
    	 while ( !queue.isEmpty() && numCompletions > 0) {
    		 ...
    		 if ( removed.endsWord()) {
    			 numCompletions--;
    			 toReturn.add(removed.getText());
    			 Set<Character> keys = removed.getValidNextCharacters();
    			 for( Character c : keys) {
    				 if ( removed.getChild(c) != null) {
